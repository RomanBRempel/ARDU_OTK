...
public sealed class AppServices
{
    ...
    private string? _observationPort;

    /// <summary>Порт, на котором сейчас установлена связь, или <c>null</c>.</summary>
    /// <remarks>
    /// Во время приёмки это порт сессии: она держит его монопольно, и
    /// наблюдательного соединения в этот момент нет.
    /// </remarks>
    public string? ConnectedPort
    {
        get => _session is { IsConnected: true } ? _session.PortName : _observationPort;
        private set => _observationPort = value;
    }
    ...
    /// </summary>
    /// <remarks>
    /// 🔴 Приёмка считается связью. Порт открывается монопольно, и на время
    /// процедуры наблюдательное соединение закрыто; если не учитывать канал
    /// сессии, приложение объявляет «связи нет» на подключённом борту, гасит
    /// приборы и блокирует кнопки по несуществующей причине.
    /// </remarks>
    public bool IsLinkConnected => _link is { IsConnected: true } || _session is { IsConnected: true };
    ...
    /// </summary>
    public VehicleLiveState? LiveState => _session is { IsConnected: true }
        ? _session.LiveState
        : _link?.LiveState;
    ...
    public Task<double> ReadParameterAsync(string name, CancellationToken ct = default) => Task.Run(
        async () =>
        {
            // Во время приёмки порт принадлежит сессии: читаем по её каналу,
            // иначе опознание компасов после перестановки очереди сорвётся с
            // жалобой «нет связи» на подключённом борту.
            if (_session is { IsConnected: true } session)
            {
                var fromSession = await session.ReadParamAsync(name, ct).ConfigureAwait(false);
                return (double)fromSession.Value;
            }
