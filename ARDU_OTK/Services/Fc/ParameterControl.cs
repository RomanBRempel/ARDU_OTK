...
public sealed class ParameterRoleMap
{
    ...
    /// </para>
    /// <para>
    /// 🔴 По умолчанию под контролем, но <b>без показа</b> оператору и прочие
    /// параметры, и компасный блок. Эталон — это конфигурация изделия целиком, в
    /// ней тысяча с лишним имён, и показывать их все значит не показывать ничего.
    /// В видимую секцию поднят только заданный технологом набор наблюдения (блок
    /// «Что оператор держит перед глазами» ниже); всё сверх него решает технолог
    /// ползунком в строке, и его решение записывается в эталон.
    /// </para>
    /// </remarks>
    ...
    [
        ...
        // 🔴 Сетевой адрес — не свойство изделия. Номер узла DroneCAN назначает
        // сеть: у динамического распределения его выдаёт сам полётный
        // контроллер, и на двух физически одинаковых изделиях он законно
        // разный. Держать его под контролем — то же самое, что сверять номер
        // шины I2C: расхождение неустранимо, оператор упирается в «расходится
        // 1» и вынужден каждый раз принимать его руками.
        new ParameterRoleRule(
            "GPS_CAN_NODEID?",
            ParameterControl.Uncontrolled,
            "@ReadOnly: номер узла DroneCAN, обнаруженный прошивкой для этого приёмника. "
          + "Назначается сетью, а не изделием, и на одинаковых бортах законно разный.",
            CannotMatch: true),
        ...
        new ParameterRoleRule(
            "INS_GYROFFS*",
            ParameterControl.Uncontrolled,
            "Смещения гироскопа первого экземпляра — имя без цифры.",
            CannotMatch: true),

        // 🔴 Калибровка инерциальных датчиков — это измерение конкретных
        // микросхем этой платы, а не настройка изделия. Перенесённая с образца,
        // она заменяет собственную калибровку платы чужими числами: борт
        // считает себя откалиброванным и уверенно врёт о своём положении,
        // причём внешне всё сходится с эталоном. Ни одна проверка стенда этого
        // не поймает — смещения ненулевые, масштаб в пределах, предполётная
        // проверка молчит. Поэтому они выведены из-под контроля и переноса:
        // калибровку акселерометров делают на самой плате.
        new ParameterRoleRule(
            "INS_ACC*OFFS_*",
            ParameterControl.Uncontrolled,
            "Смещения акселерометра этой платы. Получены калибровкой самой платы и переносу не подлежат: "
          + "чужие смещения дают борт, уверенно врущий о своём положении при полном совпадении с эталоном.",
            CannotMatch: true),
        new ParameterRoleRule(
            "INS_ACC*SCAL_*",
            ParameterControl.Uncontrolled,
            "Масштабные коэффициенты акселерометра этой платы. Получены её же калибровкой.",
            CannotMatch: true),
        new ParameterRoleRule(
            "INS_TCAL*",
            ParameterControl.Uncontrolled,
            "Температурная калибровка конкретных микросхем этой платы.",
            CannotMatch: true),
        new ParameterRoleRule(
            "AHRS_TRIM_*",
            ParameterControl.Uncontrolled,
            "Наклон установки платы, измеряемый калибровкой уровня на самом изделии. "
          + "Перенос с образца подменил бы результат собственного измерения.",
            CannotMatch: true),
        ...
        //
        // Блок, попадающий в видимую секцию по умолчанию. Это не «самые важные
        // параметры вообще», а карта наблюдения, заданная технологом: то, по чему
        // на стенде опознают перепутанную сборку и неверно снаряжённое изделие.
        ...
        .. ChannelRules("RC", RcChannelCount, ShownRcChannelCount, RcChannelReason),
        .. ChannelRules("SERVO", ServoOutputCount, ShownServoOutputCount, ServoOutputReason),

        // 🔴 Воздушная скорость. ARSPD_USE решает, слушает ли автопилот трубку
        // вообще; ARSPD_RATIO — её масштаб; ARSPD_AUTOCAL разрешает прошивке
        // менять этот масштаб в полёте. Изделие с выключенным датчиком или с
        // чужим масштабом проходит наземные проверки полностью и расходится с
        // эталоном только в воздухе. ARSPD_OFFSET сюда не входит: он снимается
        // самой платой и выведен из-под контроля правилом выше.
        new ParameterRoleRule("ARSPD_USE", ParameterControl.ControlledVisible, AirspeedReason),
        new ParameterRoleRule("ARSPD_RATIO", ParameterControl.ControlledVisible, AirspeedReason),
        new ParameterRoleRule("ARSPD_AUTOCAL", ParameterControl.ControlledVisible, AirspeedReason),

        // 🔴 Ножка измерения тока — это разводка платы, а не настройка вкуса.
        // Неверный номер даёт нулевой либо чужой ток: счётчик израсходованной
        // ёмкости врёт, отказ по батарее не срабатывает, и на стенде это не
        // видно ничем, кроме самого значения.
        new ParameterRoleRule(
            "BATT_CURR_PIN",
            ParameterControl.ControlledVisible,
            "Аналоговый вход измерения тока. Задан разводкой платы: неверный номер даёт нулевой "
          + "или чужой ток, и защита по батарее не сработает при исправном на вид изделии."),

        // 🔴 Отказоустойчивость. Имена перечислены в обоих написаниях, как и
        // пределы отклонения: у ArduPlane выдержка разнесена на короткую и
        // длинную (FS_SHORT_TIMEOUT, FS_LONG_TIMEOUT), у прочих прошивок она
        // одна (FS_TIMEOUT). Правило на несуществующее имя не срабатывает
        // никогда и вреда не делает, а недостающее правило означает молчание
        // о параметре там, где он есть.
        new ParameterRoleRule("FS_LONG_ACTN", ParameterControl.ControlledVisible, FailsafeReason),
        new ParameterRoleRule("FS_LONG_TIMEOUT", ParameterControl.ControlledVisible, FailsafeReason),
        new ParameterRoleRule("FS_SHORT_ACTN", ParameterControl.ControlledVisible, FailsafeReason),
        new ParameterRoleRule("FS_SHORT_TIMEOUT", ParameterControl.ControlledVisible, FailsafeReason),
        new ParameterRoleRule("FS_TIMEOUT", ParameterControl.ControlledVisible, FailsafeReason),

        // 🔴 Журналирование. Изделие с выключенным журналом внешне исправно и
        // ведёт себя как эталонное — до первой рекламации, разбирать которую
        // будет нечем. LOG_LASTFILE сюда не входит: его прошивка ведёт сама.
        new ParameterRoleRule("LOG_BACKEND_TYPE", ParameterControl.ControlledVisible, LoggingReason),
        new ParameterRoleRule("LOG_BITMASK", ParameterControl.ControlledVisible, LoggingReason),
    ...
    /// <summary>
    /// Сколько каналов приёмника показывать оператору по умолчанию.
    /// </summary>
    /// <remarks>
    /// 🔴 Пять, а не все шестнадцать. Контроль и показ — разные вещи: под
    /// контролем всё, а на экране только та выборка, по которой перепутанную
    /// сборку опознают с одного взгляда. Шестнадцать каналов вместе с
    /// шестнадцатью выходами дают под две сотни строк, и список перестаёт быть
    /// выборкой — в нём уже нельзя ничего заметить. Что поднять в показ сверх
    /// этого, решает технолог ползунком в карточке эталона.
    /// </remarks>
    private const int ShownRcChannelCount = 5;

    /// <summary>
    /// Сколько выходов на исполнительные механизмы показывать по умолчанию.
    /// </summary>
    /// <remarks>
    /// 🔴 Десять, а не пять, как у каналов приёмника, — числа разные намеренно.
    /// Канал приёмника задаёт раскладку пульта, и первых пяти хватает, чтобы её
    /// опознать. Выход задаёт, что физически подключено к разъёму, и на изделии
    /// занято до десяти выходов: элероны, руль высоты, руль направления, тяга,
    /// закрылки и механизация. Общее число для обеих групп означало бы, что
    /// половина сборки видна оператору, а половина нет — по причине, которой
    /// нет.
    /// </remarks>
    private const int ShownServoOutputCount = 10;
    ...
    private const string AirspeedReason =
        "Воздушная скорость: слушает ли автопилот трубку, с каким масштабом и вправе ли менять "
      + "его сам. Изделие с выключенным датчиком или чужим масштабом проходит все наземные "
      + "проверки и расходится с эталоном только в полёте.";

    private const string FailsafeReason =
        "Поведение при потере управления: что изделие сделает и через сколько. Это не настройка "
      + "удобства, а решение за оператора в момент, когда его уже не спросить.";

    private const string LoggingReason =
        "Журналирование: чем пишется журнал и что в него попадает. Изделие с выключенным журналом "
      + "внешне исправно и ведёт себя как эталонное — до первой рекламации, разбирать которую "
      + "будет нечем.";
    ...
    public static string SectionHint(ParameterControl control) => control switch
    {
        ParameterControl.ControlledVisible =>
          ...
          + "По умолчанию сюда попадает карта наблюдения технолога: каналы приёмника и выходы на "
          + "механизмы, полётные режимы, пределы отклонения, воздушная скорость, отказоустойчивость, "
          + "журналирование и вход измерения тока. Состав правится ползунком в строке — умолчание "
          + "кода не отменяет решения технолога о конкретной модели.",
        ParameterControl.ControlledHidden =>
