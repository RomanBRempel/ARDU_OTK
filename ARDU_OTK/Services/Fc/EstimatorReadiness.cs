...
public static class EstimatorReadiness
{
        ...
        IReadOnlyList<string> otherComplaints)
    {
        ...
        //
        // 🔴 Причиной объявляются только претензии к тем датчикам, на которых
        // стоит оценщик. Всё прочее, что мешает взведению — отказ радио, фикс
        // второго приёмника, питание, — к подъёму оценщика отношения не имеет,
        // и записывать его в причину значит уводить оператора не туда с
        // уверенным видом.
        var blocking = otherComplaints.Where(IsSensorComplaint).ToArray();
        ...
        if (blocking.Length > 0)
        {
            return new EstimatorDiagnosis(
              ...
              + "претензии к его датчикам: " + string.Join("; ", blocking)
        ...
        var unrelated = otherComplaints.Where(static t => !IsSensorComplaint(t)).ToArray();
        var aside = unrelated.Length == 0
            ? string.Empty
            : " Прочие претензии борта к подъёму оценщика не относятся и причиной не являются: "
            + string.Join("; ", unrelated) + ".";

        return new EstimatorDiagnosis(
            EstimatorFaultKind.Unknown,
            $"Назначен {type}, флаг включения выставлен, претензий к датчикам оценщика борт не назвал, но положение "
          + "по-прежнему считает не назначенный оценщик. Причина по доступным признакам не определена — "
          + "разберите вручную." + aside);
    }

    /// <summary>
    /// Слова, по которым претензия относится к датчикам, на которых стоит
    /// оценщик положения.
    /// </summary>
    /// <remarks>
    /// 🔴 Список намеренно узкий. Оценщик считает положение по инерциальным
    /// блокам, барометру и магнитометру; отказ радиоканала, фикс приёмника и
    /// напряжение мешают взведению, но подъёму оценщика — нет. Широкий список
    /// превратил бы шаг в машину, называющую причиной первую попавшуюся строку.
    /// </remarks>
    private static readonly string[] SensorWords =
    [
        "accel", "gyro", "compass", "mag", "baro", "imu", "ins ", "vibration", "ahrs",
    ];

    /// <summary>Относится ли претензия к датчикам оценщика.</summary>
    private static bool IsSensorComplaint(string text) =>
        !string.IsNullOrWhiteSpace(text)
        && Array.Exists(SensorWords, w => text.Contains(w, StringComparison.OrdinalIgnoreCase));
