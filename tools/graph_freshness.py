...
# Типы файлов, которые graphify берёт в корпус (см. CLAUDE.md, раздел
# «Актуальность графа знаний»). Фильтр применяется к обеим сторонам сравнения,
# поэтому прочие типы в манифесте (например LICENSE.txt) не дают ложных «удалено».
TRACKED_SUFFIXES = {
